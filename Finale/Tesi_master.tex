...
\item{\emph{Threads}/\emph{Thread} : contains a list of integer values representing the number of the core on which to schedule the task. The list will contain only one element for alla pragmas, except in case of a \emph{$\#$omp parallel for} and \emph{$\#$omp for} which are splitted.}
\item{\emph{Start\_time} : contains the start time calculated by the Chetto\&Chetto algorithm.}
\item{\emph{Deadline} : represents the daedline for the tasks execution.}
\item{\emph{Barrier}/\emph{id} : contains a list of task ids which identifies the tasks that have to synchronize after terminating the tasks execution.}
\end{itemize}
...
After the profiling step the different types of gaphs described in \ref{a}. To create the main graph, the \emph{flow graph}, which represents the execution flow of the program, the following pseudo code has been implemented:
...
\emph{getParalGraph()} creates a list of graphs, one for each function encountered in the pragma XML file created in \ref{f}; the \emph{scan()} function is called for each found function which will create and add all the pragma nodes encountered. All the informations found in the profile log, created in \ref{p} ,will be added to the node. The \emph{scan()} function has to call a special function, \emph{create\_diamond()}, when a pragma node whith nested nodes is encoutnered. This is due to the fact that special barrier nodes have to be added to maintain the OpeMP semantic. \emph{create\_diamond()} is a recursive function since there could be other pragma nodes nested inside nested pragma nodes. Each function creates not only the object graph, but also the graphical one using the \emph{pydot} library, a Python interface to the \emph{Graphviz dot} language. To visualize the graphical graphs \emph{Zgrviewer} can be used, a graph visualizer implemented in Java and based upon the \emph{Zoomable Visual Transformation Machine} \ref{a}.
...
When the main program wants to call the parallel scheduler, it creates a task input sequence by randomizing the task\_list and  creates a new process passing to it a reference to the parallel scheduler call along with the requested arguments. All the procesess are then started and the main program remains in a wait state attending the result from the shared queues; after receiving the results, all the processes are joined and the best solution of all the executions is choosen.

After finding the best schedule, it is necessary to check if it is feasible using the modified version of the Chetto\&Chetto algorithm.
...
The \emph{get\_min} function just returns the minium deadline set among all the children of a node. The \emph{get\_max} function returns the maximum deadline and arrival time found based on the criterions described in \ref{a}. \emph{all\_set} checks if all arrival times are set in the parents of a node.
After the \emph{chetto} call the main program checks if all the deadlines and arrival times are positive and in case constructs the scheduling graph taking care of adding all the necessary synchronization barriers; the schedule is sjown in \ref{a}.
